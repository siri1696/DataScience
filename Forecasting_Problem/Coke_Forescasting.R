...
library(fpp)
library(smooth)
# Loading coco-cola Data
library(readxl)
coco_data<-read_excel(file.choose())
#View(coco_data)
library(tseries)

# Converting data into time series object
coke<-ts(coco_data$Sales,frequency = 4,start=c(86))
View(coke)

# dividing entire data into training and testing data 
train<-coke[1:38]
test<-coke[39:42] # Considering only 4 Quarters of data for testing because data itself is Quarterly
# seasonal data
...
plot(coke) # Visualization shows that it has level, trend, seasonality => Additive seasonality
...
new_model <- hw(coke,alpha = NULL,beta = NULL,gamma = NULL)
